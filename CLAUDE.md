...
- **Integer `id`** (`SERIAL PRIMARY KEY`) — internal. Used for foreign keys, cache keys, JWT claims, log lines, and DB joins. **Never appears in API responses, URLs, or request bodies.** Tagged `json:"-"`.
...
JWT claims continue to embed the **int** `user_id`. The JWT is server-signed and never browser-rendered as anything but an opaque bearer string, so there's no enumeration risk and we save a lookup per request.
