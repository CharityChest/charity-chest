...
│   │   ├── handler/auth_mailer.go  # MailerGateway interface + goMailMailer/disabledMailer + password-recovery email assembly (HTML via templ, plaintext via strings.Builder)
...
│   │   ├── templates/              # Templ-rendered HTML, grouped by category (email/, future: page/ etc.)
│   │   │   └── email/              # Transactional email bodies — one file per email
│   │   │       ├── types.go        # PasswordResetData and any future per-template data structs
│   │   │       ├── password_reset.templ        # templ source for the password-reset HTML body
│   │   │       └── password_reset_templ.go     # Generated by `templ generate` — do not edit by hand
│   │   ├── testdb/                  # Shared test harness: spins up postgres:16-alpine via testcontainers-go, applies migrations to a template DB, and hands fresh per-test databases (`testdb.Open(t)`) cloned via `CREATE DATABASE ... TEMPLATE`. Dropped on cleanup with `WITH (FORCE)`. Not imported from production code.
│   │   └── routes/
...
│   │           └── routes_test.go  # E2e tests for every endpoint (full stack, real Postgres via testcontainers)
...
| HTML templates | `github.com/a-h/templ` (type-safe components; `.templ` sources compiled to `*_templ.go`) |
| Email transport | `github.com/wneessen/go-mail` (SMTP) |
| Test DB | Real Postgres in `postgres:16-alpine` via `github.com/testcontainers/testcontainers-go` — fresh per-test database cloned from a migrated template (see `internal/testdb`) |
...
- SMTP vars (all optional — when `SMTP_HOST` is unset the password-recovery email is silently skipped, the forgot-password endpoint still returns the neutral 204 response, and a server-side warning is logged; **no 503** is returned because that would be an enumeration signal): `SMTP_HOST`, `SMTP_PORT` (default `587`), `SMTP_USERNAME`, `SMTP_PASSWORD`, `SMTP_FROM`, `SMTP_FROM_NAME` (default `Charity Chest`), `SMTP_FORCE_IPV4` (default `true`). When `SMTP_HOST` is set, `SMTP_FROM` is required. `SMTP_USERNAME` and `SMTP_PASSWORD` are an **optional pair** — both or neither — so capture servers like Mailpit (which rejects AUTH outright) and internal relays work without changes; the mailer skips the AUTH step when both are empty. `Load()` reports every missing companion in the error. `SMTP_FORCE_IPV4` pins the SMTP dial to `tcp4` via a custom `go-mail` `DialContextFunc`; set it to `"false"` to allow IPv6 (rarely needed — many self-hosted relays publish unreachable AAAA records that cause the default dual-stack dial to time out).
...
# Test (requires a running Docker daemon — testcontainers-go boots a Postgres container)
make test                                          # all tests
make test-coverage                                 # combined coverage report
                                                   #   - full coverage (includes main.go/cmd/generated)
                                                   #   - business coverage (excludes them; must stay ≥ 80%)
...
# Regenerate templ-rendered HTML (run after editing any `*.templ` file).
# `make build-debug` and `make build-release` already invoke this automatically;
# the generated `*_templ.go` files are also checked in so plain `go build`
# works without a separate codegen step.
make templ
...
- **Unit tests**: one `_test.go` file per source file, in `package foo_test` (black-box). Each test gets a fresh per-test Postgres database via `newTestDB(t)` (a thin wrapper around `testdb.Open(t)` from `internal/testdb`). The first call in a test binary boots a `postgres:16-alpine` container via testcontainers-go and applies the project's SQL migrations to a template database; each subsequent call clones the template with `CREATE DATABASE ... TEMPLATE` (very fast) and drops it on `t.Cleanup`. **Docker must be running** for tests to pass.
- **E2e tests**: `internal/routes/v1/routes_test.go` exercises every endpoint through the full Echo stack (all middleware in the chain). `newServer(t)` wires `RegisterHealth` + `RegisterAuth` + `RegisterAPI` + `RegisterSystem` + `RegisterOrgs` against the same per-test Postgres database described above.
- **No testify**: tests use only the standard `testing` package.
- **Migrations**: always add a matching `.down.sql` for every `.up.sql`.
- **Cache**: selected handler groups (`AuthHandler`, `OrgHandler`, `AdminHandler`, `SystemHandler`, `ProfileHandler`) receive a `*cache.Cache`. For basic functional handler tests pass `cache.Disabled()`. For tests that specifically exercise cache hit/miss/invalidation/error paths, use an in-process `miniredis` instance via `newMiniRedisCache(t)` (defined in `auth_test.go`) — it's pure Go and runs in the test binary, so no separate container is needed for the cache (only Postgres for the DB). Cache errors are non-fatal — log with `log.Printf` and fall through to the database. Never skip cache invalidation on a successful write.
- **Templates**: all templ sources live under `internal/templates/<category>/`, one file per template (e.g. `email/password_reset.templ`). The category subdirectory is the Go package — keep it tightly scoped (`email`, `page`, …) so importers read naturally (`email.PasswordReset`). Per-template data structs live alongside the `.templ` in `types.go`. Run `make templ` after editing a `.templ` source; both the source and the generated `*_templ.go` are checked in.
