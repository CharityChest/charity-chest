// Package cache wraps a Valkey/Redis client with typed helpers.
// The operational backend has no cached endpoints in v1; the package is wired
// up so future handlers can call Get/Set/Del without re-bootstrapping.
package cache
...
// Cache wraps a Valkey/Redis client. When disabled all methods are no-ops so
...
// SCAN errors are returned to the caller; individual DEL errors are logged.
