...
| `/:locale/orgs/:orgUUID` | JWT (any member, system, root) | Org detail, member management, plan management (upgrade/enterprise/cancel) |
| `/:locale/billing/success` | — | Stripe post-checkout success page — reads `?org_uuid` from query string |
| `/:locale/billing/cancel` | — | Stripe post-checkout cancel page — reads `?org_uuid` from query string |
...
│   │       ├── orgs/[orgUUID]/page.tsx   # Protected — org detail, members, plan management
...
| `src/app/[locale]/orgs/[orgUUID]/page.test.tsx` | Org detail access control, org display, member management, plan badge, billing actions |
| `src/app/[locale]/billing/success/page.test.tsx` | Success page rendering, back-to-org link, missing org_uuid handling |
...
The API server is **not** included in this compose file. Start it separately via `docker compose -f services/admin/backend/.docker-dev/docker-compose.yml up --build` or `go run .` from the `services/admin/backend/` directory.

## Building the staging Docker image

The staging image (`services/admin/frontend/.docker-staging/Dockerfile`) is a multi-stage production build: `npm ci` in a `deps` stage, `next build` in a `builder` stage, and a lean `runner` stage that runs `next start` as the non-root `node` user.
...
# Build context is services/admin/frontend/ (the trailing argument), not the repo root.
docker build \
  -f services/admin/frontend/.docker-staging/Dockerfile \
